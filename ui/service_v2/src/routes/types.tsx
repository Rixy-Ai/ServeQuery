import type { Expect } from 'servequery-ui-lib/api/types/utils'

import type {
  ExpectEqActuall,
  GetMatches,
  GetRouteStructure
} from 'servequery-ui-lib/router-utils/types'

import type { routes } from 'routes/src'

export type Routes = GetMatches<typeof routes>

export type Paths = Routes['path']

export type GetRouteByPath<K extends Paths> = Extract<Routes, { path: K }>

export type __TESTS_ROUTE_STRUCTURE = Expect<ExpectEqActuall<GetRouteStructure<typeof routes>>>
